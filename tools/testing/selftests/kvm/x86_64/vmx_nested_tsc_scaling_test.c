...
#define SCALE_DOWN_FACTOR 2ULL  /* L1 is scaled down by this factor */
#define SCALE_UP_FACTOR   4ULL  /* L2 is scaled up (from L1's perspective) */
                                /* using this factor */

#define TSC_OFFSET_L2 (1UL << 32)
#define TSC_MULTIPLIER_L2 (SCALE_UP_FACTOR << 48)
...
static void compare_tsc_freq(uint64_t actual, uint64_t expected)
{
	...
	TEST_ASSERT(thresh_low < actual,
		"TSC is expected to be between %"PRIu64" and %"PRIu64
...
int main(int argc, char *argv[])
{
	...
	vcpu_ioctl(vm, VCPU_ID, KVM_SET_TSC_KHZ,
	           (void *) (tsc_khz / SCALE_DOWN_FACTOR));
