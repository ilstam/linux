				...
				int len, const void *val)
{
	...
	if (dev->buffer_dev)
		lock = &dev->buffer_dev->ring_lock;

	spin_lock(lock);

	if (dev->buffer_dev) {
		ring = dev->buffer_dev->ring;
		if (!ring || !dev->buffer_dev->ring_size) {
			spin_unlock(lock);
			return -EOPNOTSUPP;
		}

		num_entries = KVM_COALESCED_MMIO_MAX2(dev->buffer_dev->ring_size);
	}
...
static int coalesced_mmio_buffer_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	size_t min_size = sizeof(struct kvm_coalesced_mmio_ring) + sizeof(struct kvm_coalesced_mmio);
	unsigned long pfn;
	int ret = 0;
	...
	dev->ring = vmalloc_user(requested_size);
	if (!dev->ring) {
		ret = -ENOMEM;
		goto out_unlock;
	}
	...
	if (remap_pfn_range(vma, vma->vm_start, pfn, requested_size, vma->vm_page_prot)) {
		vfree(dev->ring);
		dev->ring = NULL;
		ret = -EAGAIN;
		goto out_unlock;
...
int kvm_vm_ioctl_create_coalesced_mmio_buffer(struct kvm *kvm)
{
	...
	dev = kzalloc(sizeof(struct kvm_coalesced_mmio_buffer_dev), GFP_KERNEL_ACCOUNT);
	if (!dev)
		return -ENOMEM;
	...
	ret = anon_inode_getfd("coalesced_mmio_buf", &coalesced_mmio_buffer_ops, dev, O_RDWR | O_CLOEXEC);
	if (ret < 0) {
		kfree(dev);
		return ret;
	}
