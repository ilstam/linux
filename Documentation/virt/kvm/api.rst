...
KVM_(UN)REGISTER_COALESCED_MMIO2 should be used instead if available.
...
The fd must be first passed to mmap() to allocate a ring buffer of the desired
size that is shared between kernel and userspace. The ring buffer must be
...
same to the first index after consuming entries.
...
The minimum buffer size to be requested from mmap() must be `sizeof(struct
kvm_coalesced_mmio_ring) + 1 * sizeof(struct kvm_coalesced_mmio)`.
